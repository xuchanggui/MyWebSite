...
function loginDialog(){
   var loginTpl = '<div class="m-login-register">\
                  <div class="inner">\
                    ...
                    <div class="form-item change-pt">\
                         <form action="/user-ajax_login" wx-validator name="loginForm" wx-validator-ajax wx-validator-single-error="loginFormError">\
                             ...
                             <p class="p-login">\
                               ...
                               <a class="weibo" href="https://open.t.qq.com/cgi-bin/oauth2/authorize?client_id=801393846&redirect_uri=http%3A%2F%2Fwww.zhongchou.cn%2Fapi_callback.php%3Fc%3DTencent&response_type=code"></a>\
                             </p>\
                         </form>\
                   </div>\
                   <p class="bottom-style">还没有帐号？<a id="switchRegister" class="blue-color" href="#">立即注册</a></p>\
                 </div>\
         </div>';

  var registerTpl = '<div class="m-login-register">\
                  <div class="inner">\
                    <p class="p-shut-down clearfix"><a class="shut-down-icon Js-pop-close"></a></p>\
                    <h2>注册</h2>\
                    <div class="form-item change-pt">\
                         <form action="/user-ajax_register" wx-validator name="registerForm" wx-validator-ajax wx-validator-single-error="registerFormError">\
                             ...
                             <div class="row clearfix">\
                              ...
                              <div class="row clearfix">\
                                 <input name="user_pwd" type="password" wx-validator-rule="required|rangelength" wx-validator-param="|6-16" wx-validator-placeholder="请输入6-16位密码">\
                             </div>\
                             <div class="row clearfix">\
                 ...
                 <p class="bottom-style">已有帐号？立即 <a id="switchLogin" class="blue-color" href="###">登录</a></p>\
                 </div>\
         </div>';
  ...
  $("#Js-registerGetCode").live("click",function(){
    if($(this).hasClass('Js-hasSendCode'))
      return;
    
    var v_code = $("#v_code").val(),
        $error = $("#registerFormError"),
        query = "mobile=" + $("input[name='mobile']").val() + "&csrf_token=" + ($("#csrf_token").val()||"")+"&v_code="+v_code;
    ...
    wx.deferred().then(function(next){
      ...
      wx.sendData("/user-ajax_registercode",query ,function(data){
        if(data.status){
